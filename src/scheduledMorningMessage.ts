...
const postAbsencesMessage = () => {
  ...
  absenceResponse().pipe(
      ...
      tap(response => console.log(new Date(), 'posted daily morning message, res: ', response)),
      catchError(e => {
        console.error(new Date(), 'failed to post daily morning message, res: ', e);
        return of(undefined);
      })
  ).subscribe();
};

console.log(new Date(), 'scheduled post daily afternoon message');
